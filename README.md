...
`data.default.js` 를 코드에서 고친 뒤, 관리자에서 **최신 내용 불러오기** 를 누르면 **배포본이 바뀐 부분만** 반영된다. 관리자에서 직접 고친 내용은 그대로 남는다.

판단 기준은 저장된 문서 안의 **기준본**(`__base`, 지난번에 불러온 배포본 사본)이다.

| 상황 | 결과 |
|---|---|
| 배포본만 바뀜 | 최신 내용으로 갱신 |
| 내가 관리자에서만 고침 | 내 내용 유지 |
| 배포본에 새 항목 추가 | 추가 반영 |
| 내가 추가한 항목 | 유지 |
| 내가 지운 항목 | 되살리지 않음 |
| 배포본에서 빠진 항목 | 내가 안 건드렸으면 삭제, 고쳤으면 유지 |
| 같은 곳을 양쪽이 고침 | 최신으로 덮고, 확인창에 몇 곳인지 표시 |

항목 대응은 이름으로 맞춘다 — 프로젝트는 `name`, 경력은 `org`, 학력은 `school`, 숫자 타일은 `label`. 따라서 **프로젝트명을 바꾸면 다른 항목으로 인식**되어 새로 추가된다.

적용 전 상태는 자동으로 스냅샷되므로 결과가 마음에 들지 않으면 **되돌리기** 를 누르면 된다. 기준본이 없는 첫 실행은 예전 방식(섹션 통째 교체)으로 한 번 진행한 뒤 기준본을 저장한다.
